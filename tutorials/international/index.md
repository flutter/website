...
app is rebuilt with a new DemoLocalizations object, one
...
delegates. `DemoLocalizations.of()` only finds the app's
Localizations widget if its context is a child of the app, so the
the app's `home` is a separate widget.
