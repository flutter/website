...
Flutter tooling currently expects iOS plugins to expose an Objective-C header. But you can
implement your plugin in Swift instead and just have the Objective-C part forward plugin
registration to that implementation. This section describes how to change the Objective-C
plugin generated by `flutter create --plugin hello` to a Swift-based implementation.

Before editing the iOS platform code in Xcode, first make sure that
the code has been built at least once by Flutter by executing
...
1. Select 'File > Open', and select the `hello/example/ios/Runner.xcworkspace` file.
   The iOS platform code of your plugin is located in `Pods/Development Pods/hello/Classes/` in the Project Navigator.
1. Select the `Classes` folder, then select `File > New > File...` and add a new Cocoa Touch class.
   Name the class `RealHelloPlugin`, make it a sub-class of `NSObject`, and select `Swift` as language.
   Press `Next` and accept the location for the new class.
1. Replace the code for the new class with
    ```swift
    import Flutter
    import UIKit
    
    public class RealHelloPlugin: NSObject, FlutterPlugin {
        public static func register(with registrar: FlutterPluginRegistrar) {
            let channel = FlutterMethodChannel(name: "hello", binaryMessenger: registrar.messenger());
            let instance = RealHelloPlugin();
    ...
    ```
1. Replace the code of `HelloPlugin.m` with a forwarding call:
    ```objective-c
    ...
    + (void)registerWithRegistrar:(NSObject<FlutterPluginRegistrar>*)registrar {
        [RealHelloPlugin registerWithRegistrar:registrar];
    }
    @end
    ```
1. Build the project again using Flutter by executing `flutter build ios` in
   the terminal.

You can now run the example app from the terminal by executing `flutter run`
or from Xcode by pressing the &#9654; button.
