...
plugin generated by `flutter create --plugin hello` into a Swift-based implementation.
...
1. Select 'File > Open', and select the `hello/example/ios/Runner.xcworkspace` file.
   The iOS platform code of your plugin is located in `Pods/Development Pods/hello/Classes/` in the Project Navigator.
1. Select the `Classes` folder, then select `File > New > File...` and add a new Swift file named
   `RealHelloPlugin.swift`.
1. Replace the contents of the new file with:
    ```swift
    import Flutter
    import UIKit
...
1. Replace the implementation in `HelloPlugin.m` with a forwarding call:
    ...
    @end
    ```
1. The example app needs to be rebuilt by Flutter before it can be
   launched from Xcode. Execute either `flutter build ios`
   or `flutter run` in the terminal.
   
Following these steps, you can run the example app as usual from Xcode by
pressing the &#9654; button.
