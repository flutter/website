...
Note that the child's layout cannot depend on its position, as the position
is described by the parent only after it returns from the layout. As a
result, the parent is free to reposition the child without needing to
recompute the child's layout.
