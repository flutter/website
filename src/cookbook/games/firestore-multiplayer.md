---
title: Add multiplayer support via Firestore
description: >
...
For example, real-time strategy games run --- as the name suggests ---
in real-time, but that doesn't correlate to a high tick rate.
...
Write your game code to allow changing the game state in response to both
local events and remote events.
...
is a horizontally scaling NoSQL document database in the cloud.
...
As soon as you get the step of writing Dart code in that guide,
...
The most common issues you might encounter when testing Firebase integration
...
At this point, the game has near-instant and dependable synchronization
of state across clients.
...
follow [a Firestore codelab or two](https://codelabs.developers.google.com/?product=flutter&text=firestore)
...
As you approach the release date, think about authentication and match-making.
Thankfully, Firebase provides a
[built-in way to authenticate users](https://firebase.google.com/docs/auth/flutter/start)
and our Firestore database structure can handle multiple matches.
