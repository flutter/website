...
| `[no-]relax-syntax`                 | When specified, the syntax will be relaxed so that the special character "{" is treated as a string if it is not followed by a valid placeholder and "}" is treated as a string if it does not close any previous "{" that is treated as a special character. |
| `[no-]use-named-parameters`         | Whether or not to use named parameters for the generated localization methods. |
