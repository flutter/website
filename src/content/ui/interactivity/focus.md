...
**Focus scope**
  ...
  While it limits keyboard-initiated focus traversal to those nodes,
  it allows focus to be assigned to nodes outside of its scope
  if the user clicks on that node.
...
To just listen, you can also use the `KeyboardListener` widget.
...
Consider how a simple grid layout decides the next control.
...
Common practice would be to not call the member functions of the class directly.
