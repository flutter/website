...
A boolean flag to force the Skia WebAssembly (skwasm) renderer to run in **single-threaded mode**. This is useful if:

- Your environment does not support multi-threaded WASM (e.g., SharedArrayBuffer not available or required security headers missing).  
